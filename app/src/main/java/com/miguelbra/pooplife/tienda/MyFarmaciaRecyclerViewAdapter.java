...
public class MyFarmaciaRecyclerViewAdapter extends RecyclerView.Adapter<MyFarmaciaRecyclerViewAdapter.ViewHolder> {

    private final List<Medicamento> mValues;
    private final FarmaciaFragment.OnListFragmentInteractionListener mListener;

    public MyFarmaciaRecyclerViewAdapter(List<Medicamento> items, FarmaciaFragment.OnListFragmentInteractionListener listener) {
        mValues = items;
        mListener = listener;
    }
        ...
        holder.mAñadir.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(Integer.parseInt(holder.mCantidad.getText().toString()) != 0)
                    mListener.onListFragmentInteraction( holder.mMedicamento, Integer.parseInt(holder.mCantidad.getText().toString()) );
            }
        });
